...
Audio::Audio() {
    ...
    Mix_Music *background = Mix_LoadMUS("client_src/audio/background.wav");
    Mix_VolumeMusic(20);
    Mix_PlayMusic(background, -1);
}

void Audio::play(std::string filename) {
    std::string file = "client_src/audio/" + filename + ".wav";
...
void Audio::ending() {
    Mix_HaltMusic();
    Mix_Music *background = Mix_LoadMUS("client_src/audio/ending.wav");
